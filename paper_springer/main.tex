...
Selected settings & LightGBM: binary objective, 31 leaves, min child samples 100, subsample/colsample 0.85, \(\lambda_2=3.0\), class weights \(\{0:1,1:2.6216\}\). XGBoost: binary:logistic objective, AUC-PR eval metric, min child weight 50, subsample/colsample 0.85, \(\lambda_2=3.0\), scale\_pos\_weight 2.6216, tree method hist. Seed 42 is used for sampling and model fitting \\
...
The final settings are read from the LightGBM, XGBoost, and ensemble run summaries under \texttt{data/processed/model\_results/}; parameters not passed by the scripts, including XGBoost \(\gamma\) and \(\lambda_1\), retain library defaults.
...
\midrule
Historical demand only & 0.2702 & 0.3513 & 0.2595 & 0.5434 & 0.2838 & 0.7374 \\
Historical + calendar & 0.3072 & 0.3785 & 0.2873 & 0.5546 & 0.3262 & 0.7635 \\
Historical + calendar + weather & 0.3143 & 0.3767 & 0.2843 & 0.5581 & 0.3246 & 0.7615 \\
Historical + calendar + weather + OSM & 0.3189 & 0.3818 & 0.2915 & 0.5535 & 0.3314 & 0.7673 \\
Historical + calendar + weather + PLUTO & 0.3165 & 0.3813 & 0.3008 & 0.5208 & 0.3299 & 0.7668 \\
...
Feature-set composition is fixed by the feature-set JSON. The history-only row includes complaint category, borough, current \texttt{complaint\_count}, lag/difference/rolling/ratio demand features, and history-availability flags; complaint category and current demand are retained in every ablation. Calendar and weather are cumulative additions, OSM and PLUTO are alternative additions after weather, and full context includes both OSM and PLUTO.
...
\caption{Top 15 individual features by mean absolute SHAP value for the tuned LightGBM explanation component. Blue bars are historical temporal features; orange bars are other feature groups.}
\label{fig:shap_top_features}
\end{figure}

Two local examples illustrate how explanations should be used. A true-positive housing alert in Inwood for feature week 2025-10-06 is associated with eight-week volatility, seasonal timing, and residential built-environment features. A false-positive public-safety alert in the Lower East Side for feature week 2025-06-30 is lifted by annual and recent lag counts and current reported demand, while other terms push the score downward. These cases support analyst review but do not establish causal effects.
