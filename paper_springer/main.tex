...
The top individual features are also consistent with the next-week abnormal reported-demand formulation. The leading features include \texttt{rolling\_8w\_std}, \texttt{ratio\_to\_8w\_mean}, \texttt{week\_of\_year}, \texttt{lag\_52w\_count}, \texttt{rolling\_4w\_mean}, \texttt{lag\_1w\_count}, one-hot service-category indicators such as \texttt{complaint\_category=environment}, and current \texttt{complaint\_count}. These features combine short-term volatility, deviation from recent baseline, annual seasonality, service domain, and current observed demand.

\subsection{Category-specific interpretation}

Category-specific SHAP summaries indicate that the highest-ranked features are broadly similar across categories, but their relative magnitudes differ. In every category, the top features include rolling eight-week standard deviation, ratio to the eight-week mean, complaint-category indicators, 52-week lag count, and week of year. This pattern supports two interpretations. First, the model is primarily learning temporal abnormality relative to local historical behavior. Second, service category remains important even after category-specific series are included, suggesting that service domains differ in baseline volatility, seasonality, and alertability.
