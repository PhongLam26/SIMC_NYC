...
Full-row ablations show that history and current count provide most of the signal, while calendar and weather add smaller increments (Table \ref{tab:ablation}). Weather does not improve validation PR-AUC over history plus calendar in this pass, so the weather variables should be read as contextual controls rather than a core contribution. In a five-seed rerun of the key ablation rows, the final borough model has mean test PR-AUC 0.3185 (SD 0.0023) and the NTA fixed-effect variant has 0.3188 (SD 0.0011), so the spatial fixed-effect gain remains too small to change the simpler final-model claim.
