...
Supported models:
    - lightgbm              optional, requires lightgbm; GPU via LightGBM GPU/OpenCL when --use-gpu
    - xgboost               optional, requires xgboost; CUDA via device='cuda' when --use-gpu
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use GPU mode for GPU-capable models. "
            "XGBoost uses CUDA with device='cuda'. "
            "LightGBM uses device_type='gpu' via GPU/OpenCL build. "
            "Scikit-learn tree models remain CPU-only."
        ),
    )
    parser.add_argument(
        "--gpu-only",
        action="store_true",
        help="Skip CPU-only models and run only GPU-capable models: xgboost/lightgbm.",
    )
    parser.add_argument(
        "--gpu-device-id",
        type=int,
        default=0,
        help="GPU device id. XGBoost uses cuda:<id>; LightGBM uses gpu_device_id.",
    )
    parser.add_argument(
        "--lightgbm-gpu-platform-id",
        type=int,
        default=-1,
        help="Optional LightGBM gpu_platform_id. -1 means do not set it.",
    )
    parser.add_argument(
        "--xgboost-gpu-mode",
        type=str,
        ...
        help=(
            "modern: tree_method='hist', device='cuda'. "
            "legacy: tree_method='gpu_hist'. Use legacy only if your xgboost version is old."
        ),
    )
...
def make_model(
    ...
    if model_name == "lightgbm":
        ...
        if args.use_gpu:
            # LightGBM GPU in the Python wheel commonly uses the GPU/OpenCL backend.
            # On NVIDIA GPUs, this still uses the GPU, but Task Manager may show it
            # under Compute/3D depending on the driver.
            params["device_type"] = "gpu"
            params["gpu_device_id"] = int(args.gpu_device_id)
            if int(args.lightgbm_gpu_platform_id) >= 0:
                params["gpu_platform_id"] = int(args.lightgbm_gpu_platform_id)
    ...
    if model_name == "xgboost":
        ...
        if args.use_gpu:
            if args.xgboost_gpu_mode == "legacy":
                # For older XGBoost builds.
                params["tree_method"] = "gpu_hist"
                params["gpu_id"] = int(args.gpu_device_id)
            else:
                # Modern XGBoost GPU path.
                params["tree_method"] = "hist"
                params["device"] = f"cuda:{int(args.gpu_device_id)}"
        else:
            params["tree_method"] = "hist"
...
def make_run_summary(
    *,
    ...
    summary = {
        ...
        "xgboost_gpu_mode": args.xgboost_gpu_mode,
        "lightgbm_gpu_platform_id": int(args.lightgbm_gpu_platform_id),
...
def main() -> None:
    ...
    if args.gpu_only:
        args.models = [m for m in args.models if m in {"lightgbm", "xgboost"}]
        if not args.models:
            raise ValueError("--gpu-only was set, but no GPU-capable models were requested. Use --models xgboost lightgbm.")

    models_to_run, skipped_models = available_models(args.models)

    if args.use_gpu:
        for m in list(models_to_run):
            if m not in {"lightgbm", "xgboost"}:
                skipped_models.append({
                    "model_name": m,
                    "reason": "cpu_only_model_skipped_when_use_gpu",
                    "note": "scikit-learn HistGradientBoosting/RandomForest/ExtraTrees do not train on CUDA GPU.",
                })
                models_to_run.remove(m)
